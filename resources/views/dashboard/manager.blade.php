        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-12 col-md-6 col-xl-3">
                <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                    ...
                    <h2 class="fw-black mb-0 text-orange">{{ $activeOrdersCount }} <small class="fs-6 text-muted">Pesanan</small></h2>
                </div>
            </div>
            <div class="col-12 col-md-6 col-xl-3">
                <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                    <div class="d-flex justify-content-between align-items-start">
                        ...
                        <button class="btn btn-sm btn-brand rounded-circle" data-bs-toggle="modal" data-bs-target="#addTableModal">
                            <i class="fas fa-plus"></i>
                        </button>
                    </div>
                    <h2 class="fw-black mb-0 text-dark">{{ $occupiedTables }} / {{ $tables->count() }} <small class="fs-6 text-muted">Meja</small></h2>
                </div>
            </div>
            <div class="col-12 col-md-6 col-xl-3">
                <div class="card border-0 shadow-sm rounded-4 p-4 h-100 bg-danger-soft border-start border-danger border-4">
                    ...
                    <h2 class="fw-black mb-0 text-danger">{{ $delayedOrders->count() }} <small class="fs-6">Order > 15m</small></h2>
                </div>
            </div>
        </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge rounded-pill px-3 {{ 
                                            $order->status === 'pending' ? 'bg-warning-soft text-warning' : 
                                            ($order->status === 'preparing' ? 'bg-primary-soft text-primary' : 'bg-success-soft text-success') 
                                        }}">
                                            {{ strtoupper($order->status) }}
                                        </span>
                                    </td>
                                    <td class="text-end pe-4 small text-muted">{{ $order->created_at->diffForHumans() }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4">
                    ...
                    <div class="alert bg-danger-soft border-0 rounded-4 d-flex align-items-center mb-0">
                        ...
                        <div>
                            ...
                            <div class="small text-muted">{{ $delayedOrders->count() }} order di kitchen macet</div>
                        </div>
                    </div>
                    @endif
                </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-light text-dark border">{{ strtoupper($tx->payment_method) }}</span></td>
                            <td class="text-end pe-4">
                                <div class="small fw-bold">{{ $tx->kasir ? $tx->kasir->name : 'System' }}</div>
                                <div class="text-muted" style="font-size: 10px;">Waiter: {{ $tx->waiter ? $tx->waiter->name : '-' }}</div>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    @elseif($tab === 'menu')
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div class="card-header bg-white py-4 px-4 border-0 d-flex justify-content-between align-items-center">
                ...
                <input type="text" class="form-control w-25 rounded-pill" placeholder="Cari menu...">
            </div>
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="badge rounded-pill px-3 {{ $item->active ? 'bg-success text-white' : 'bg-danger text-white' }}">
                            ...
                            <td class="text-end pe-4">
                                <form action="{{ route('manager.menu.toggle', $item->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-sm {{ $item->active ? 'btn-outline-danger' : 'btn-outline-success' }} rounded-pill px-3">
            ...
            <div class="col-12 col-xl-6">
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
                    <div class="card-header bg-dark text-white py-4 px-4 border-0">
                        <h5 class="mb-0 fw-bold"><i class="fas fa-boxes me-2 text-warning"></i> Permintaan Restock Gudang</h5>
                    </div>
                        ...
                        <div class="list-group-item p-4 border-bottom">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <div>
                                    ...
                                    <div class="small text-muted">Dari: {{ $req->user->name }} • {{ $req->created_at->diffForHumans() }}</div>
                                </div>
                                <div class="text-end">
                                    <div class="fw-black text-orange fs-5">{{ number_format($req->quantity, 2) }} {{ $req->ingredient->unit }}</div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex gap-2">
                                <form action="{{ route('manager.restock.approve', $req->id) }}" method="POST" class="flex-grow-1">
                                    @csrf
                                    <button class="btn btn-success w-100 rounded-3 fw-bold">APPROVE</button>
                                </form>
                                <form action="{{ route('manager.restock.reject', $req->id) }}" method="POST" class="flex-grow-1">
                                    @csrf
            ...
            <div class="col-12 col-xl-6">
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
                    <div class="card-header bg-danger text-white py-4 px-4 border-0">
                        <h5 class="mb-0 fw-bold"><i class="fas fa-shield-alt me-2"></i> Approval Void / Refund / Discount</h5>
                    </div>
                    <div class="list-group list-group-flush">
                        <!-- Simulated Void Request -->
                        <div class="list-group-item p-4 border-bottom bg-light-soft">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <span class="badge bg-danger rounded-pill px-2 small mb-2">VOID REQUEST</span>
                                <span class="small text-muted">2 Menit yang lalu</span>
                            </div>
                            <h6 class="fw-bold text-dark mb-1">Order #AB12 (Meja 05)</h6>
                            <p class="small text-muted mb-3">Alasan: Customer salah pesan, ingin ganti menu.</p>
                            <div class="d-flex gap-2">
                                <button class="btn btn-sm btn-danger flex-grow-1 rounded-3 fw-bold">APPROVE VOID</button>
                                <button class="btn btn-sm btn-outline-secondary flex-grow-1 rounded-3 fw-bold">REJECT</button>
                            </div>
                        </div>
                        
                        <!-- Simulated Discount Request -->
                        <div class="list-group-item p-4 border-bottom">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <span class="badge bg-primary rounded-pill px-2 small mb-2">SPECIAL DISCOUNT</span>
                                <span class="small text-muted">15 Menit yang lalu</span>
                            </div>
                            <h6 class="fw-bold text-dark mb-1">Order #XY99 (Meja 12)</h6>
                            <p class="small text-muted mb-3">Request: Diskon 15% (Majar Priority Member)</p>
                            <div class="d-flex gap-2">
                                <button class="btn btn-sm btn-primary flex-grow-1 rounded-3 fw-bold">APPROVE DISCOUNT</button>
                                <button class="btn btn-sm btn-outline-secondary flex-grow-1 rounded-3 fw-bold">REJECT</button>
                            </div>
                        </div>
                    </div>
                    <div class="p-4 text-center">
                        <small class="text-muted italic">Menunggu request lainnya dari Kasir...</small>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="d-flex align-items-center">
                                    <div class="user-avatar me-3 bg-brand" style="width:40px;height:40px;">{{ strtoupper(substr($s->name, 0, 1)) }}</div>
                                    <div class="fw-bold text-dark">{{ $s->name }}</div>
                                </div>
                            </td>
                            <td><span class="badge bg-light text-dark border">{{ strtoupper($s->role) }}</span></td>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-muted small">{{ number_format($ing->min_stock, 2) }} {{ $ing->unit }}</td>
                            <td class="text-end pe-4">
                                @if($ing->stock <= 0)
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4">
                    ...
                    <form action="{{ route('manager.coupon.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="value" class="form-control rounded-3" placeholder="Contoh: 10" required>
                        </div>
                        <div class="mb-4">
                        ...
                        <button type="submit" class="btn btn-dark w-100 py-3 rounded-3 fw-bold">BUAT KUPON SEKARANG</button>
                    </form>
                </div>
            </div>
            <div class="col-12 col-xl-8">
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge rounded-pill px-3 {{ 
                                    ...
                                    ($o->status === 'preparing' ? 'bg-primary text-white' : 'bg-success text-white') 
                                }}">
                                    {{ strtoupper($o->status) }}
                                </span>
                            </td>
                            <td class="text-end pe-4">
                                <span class="fw-bold {{ $o->created_at->diffInMinutes() > 20 ? 'text-danger' : 'text-dark' }}">
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-brand">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="mb-0 text-dark opacity-75">Klik meja untuk menghapus, atau gunakan tombol di bawah untuk tambah/gabung.</p>
                        </div>
                        <div class="d-flex gap-2">
                            <button class="btn btn-dark px-4 rounded-pill fw-bold" data-bs-toggle="modal" data-bs-target="#addTableModal">
                                <i class="fas fa-plus me-2"></i> Tambah Meja
                            </button>
                            <button class="btn btn-outline-dark px-4 rounded-pill fw-bold" data-bs-toggle="modal" data-bs-target="#mergeTableModal">
            ...
            <div class="col-12">
                <div class="row g-3">
                    ...
                    <div class="col-6 col-md-4 col-lg-3 col-xl-2">
                        <div class="card border-0 shadow-sm rounded-4 p-4 text-center h-100 transition-all hover-shadow {{ 
                            $table->status === 'available' ? 'bg-white border-2 border-success-soft' : 
                            ($table->status === 'occupied' ? 'bg-light border-2 border-gray-200' : 'bg-orange-soft border-2 border-orange-200') 
                        }}">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <div class="badge {{ 
                                    $table->status === 'available' ? 'bg-success' : 
                                    ($table->status === 'occupied' ? 'bg-secondary' : 'bg-warning') 
                                }} rounded-pill px-2 py-1 small">
                                    {{ strtoupper($table->status) }}
                                </div>
                                @if($table->status === 'available')
                                <form action="{{ route('manager.table.delete', $table->id) }}" method="POST" onsubmit="return confirm('Hapus meja ini?')">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <div class="mb-3">
                                <i class="fas fa-th-large fs-1 {{ $table->status === 'available' ? 'text-success' : 'text-muted' }}"></i>
                            </div>
                ...
                <div class="modal-content border-0 rounded-4 shadow">
                    <form action="{{ route('manager.table.merge') }}" method="POST">
                        ...
                        <div class="modal-body p-4">
                            <p class="small text-muted mb-4">Meja yang digabung akan dihapus dan kapasitasnya ditambahkan ke meja utama.</p>
                            <div class="mb-3">
                                <label class="form-label small fw-bold">Meja Utama (Tetap Ada)</label>
                                <select name="main_table_id" class="form-select rounded-3" required>
                                    @foreach($tables->where('status', 'available') as $t)
                                        <option value="{{ $t->id }}">{{ $t->name }} ({{ $t->seats }} kursi)</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-0">
                                <label class="form-label small fw-bold">Meja yang Digabung (Akan Dihapus)</label>
                                <select name="merge_table_id" class="form-select rounded-3" required>
                                    @foreach($tables->where('status', 'available') as $t)
                                        <option value="{{ $t->id }}">{{ $t->name }} ({{ $t->seats }} kursi)</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="modal-footer border-0 pt-0">
                            <button type="submit" class="btn btn-brand w-100 py-3 rounded-3 fw-bold shadow-sm">Proses Gabung Meja</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <form action="{{ route('manager.table.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control rounded-3" placeholder="Contoh: Meja 15 atau VIP 1" required>
                    </div>
                    <div class="mb-0">
                        <label class="form-label small fw-bold">Kapasitas Kursi</label>
                        <input type="number" name="seats" class="form-control rounded-3" placeholder="Contoh: 4" min="1" required>
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="submit" class="btn btn-brand w-100 py-3 rounded-3 fw-bold shadow-sm">Simpan Meja</button>
                </div>
            </form>
        </div>
    </div>
